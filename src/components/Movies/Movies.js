...
function Movies( {setSearchQuery, searchQuery, handleSubmit, isSubmitting, cards, notMovies, onCardLike } ) {
  ...
  return (
    <div className={`movies ${openForm ? "movies_type_dark":""}`} >
     ...
     {isSubmitting ? <Preloader /> : <MoviesCardList cards={cards} deleteMovies={deleteMovies} notMovies={notMovies} onCardLike={onCardLike} />}
    </div>
  );
}
